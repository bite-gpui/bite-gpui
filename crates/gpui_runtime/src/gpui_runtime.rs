//! GPUI's runtime: the reactive store, the windows, and the declarative element
//! DSL that composes them.
//!
//! This crate holds [`App`], [`Window`] and the context types, the entities and
//! views built on them, and the element tree that renders through the engine
//! contracts in [`gpui_engine`] onto the windowing SPI in [`gpui_platform`].
//!
//! The element DSL is not separable from the runtime: an element is laid out and
...
// `gpui` is an alias for `gpui_runtime`.
...
pub use test::*;
pub use text_system::*;
pub use util::{FluentBuilder, FutureExt, Timeout};
pub use view::*;
pub use window::*;
...
///
/// These live here rather than in `gpui_types` because they require [`App`], while
